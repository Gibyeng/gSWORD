...
// update p arr after some runs,
// p is an accumulation
void updateByEmb (double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double * p_temp){
	double sum = 0;
	// find the smallest p
	double smallestp = p[0];
	for (int i = 1; i<length; ++ i) {
		if(p[i] < smallestp){
			smallestp = p[i];
		}
	}
	// for seen node p = aver of all seened values

	for(int i = 0; i<length; ++i ){
		// average p by its count
		if(sampled_count_all[i] >= 50 && sampled_count[i] > 0 ){
			if(ac_emb[i] > 0){
				p[i] = ac_emb[i] /sampled_count[i];
			}
			else{
				p[i] = smallestp/2;
			}
	...
	if(sum == 0){
		// reset p
		std::memcpy (p,p_temp, sizeof(double)*length);
	}
	//for unseen node p = aver of all ps
	if(sum > 0){
		for(int i = 0; i<length; ++i ){
			// average p by its count
			if( sampled_count_all[i] < 50 ){
				p[i] = sum/It_count;

			}
		}
	}else{
		//do not find any vaild path.
	}

}

void updateByEmb_random (double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double & memory_p,double* p_temp){
	double sum = 0;
	// for seen node p = aver of all seened values
	for(int i = 0; i<length; ++i ){
		// average p by its count
	...
	if(sum > 0){
		for(int i = 0; i<length; ++i ){
			// copy p from an seen p randomly
			if( sampled_count_all[i] < 50 ){
				p[i] = memory_p;
			}
		}
...
void updateByEmb_random_trust (double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double & memory_p,double* p_temp){
	double sum = 0;
	int memory_count = 0;
	for(int i = 0; i<length; ++i ){
		// average p by its count
		if( sampled_count[i] > 0 )
		{
			p[i] = ac_emb[i] /sampled_count[i];
			if(ac_emb[i] != 0){
				memory_count += 1;
		...
		for(int i = 0; i<length; ++i ){
			if( sampled_count[i] > 0 )
			{
				if(ac_emb[i] != 0){
					random--;
					if(rand == 0){
						memory_p =  ac_emb[i] /sampled_count[i];
					}
				}
			}
		}
	...
	for(int i = 0; i<length; ++i ){
		...
		if(sum > 0){
			for(int i = 0; i<length; ++i ){
				// when we can not find an embedding even we try 50 times
				if( ac_emb[i] == 0  ){
					p[i] = memory_p;
				}
			}
...
void updateByEmb_minSeenNonZero (double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double & memory_p,double* p_temp){
	double sum = 0;
	for(int i = 0; i<length; ++i ){
		// average p by its count
		if( sampled_count[i] > 0 )
...
// decrease p when sampling increase
void updateByEmb_dec_p (double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double & memory_p,double* p_temp){
	double sum = 0;
	int memory_count = 0;
	for(int i = 0; i<length; ++i ){
		// average p by its count
	...
	if(sum > 0){
		for(int i = 0; i<length; ++i ){
			// when we can not find an embedding even we try 50 times
			if( ac_emb[i] == 0 ){
				if(sampled_count_all[i] == 0){
					p[i] = memory_p;
				}else{
					p[i] = memory_p* (1- sampled_count[i]/ sampled_count_all[i]);
				}
...
void updateByEmb_balance (double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double & memory_p,double* p_temp, double alpha){
	double sum = 0;
	int memory_count = 0;
	for(int i = 0; i<length; ++i ){
		// average p by its count
		if( sampled_count[i] > 0 )
		{
			...
			p[i] -= alpha* (p[i] - ac_emb[i] /sampled_count[i]);
...
void updateByEmb_rank(double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double & memory_p,double* p_temp, double alpha){
	double sum = 0;
	int memory_count = 0;
	for(int i = 0; i<length; ++i ){
		// average p by its count
		if( sampled_count[i] > 0 )
		{
			int estInt = ac_emb[i] /sampled_count[i];
			int numDigits = 1;
			while (estInt >= 10){
				numDigits++;
				estInt = estInt/10;
...
void updateByEmb_CN (double* &p, double* ac_emb ,int* sampled_count, int* sampled_count_all, ui length, int It_count, double & memory_p,double* p_temp){
	double sum = 0;
	// for seen node p = aver of all seened values
	for(int i = 0; i<length; ++i ){
		// average p by its count
	...
	if(sum > 0){
		for(int i = 0; i<length; ++i ){
			// choose the index sharing most common neighbor
			if( sampled_count_all[i] < 50 ){
				p[i] = memory_p;
			}
		}
...
void updateByEmb (double* &p, double* ac_emb ,double* ac_wl ,int* sampled_count, int* sampled_count_all, ui length, int It_count){
	double sum = 0;
	// for seen node p = aver of all seened values
	for(int i = 0; i<length; ++i ){
		// average p by its count
		if(sampled_count_all[i] >= 50 && sampled_count[i] > 0 &&ac_wl[i] > 0 ){
			p[i] = ac_emb[i] /(sampled_count[i] *ac_wl[i]);
		}
		sum += p[i];
...
void adjustPoss (double* &p, double* ac_emb_all , int* sampled_count_all, ui length){
	for (ui i =0; i < length; ++i){
		p[i] = ac_emb_all[i] / sampled_count_all[i];
	}
}

// return an estimation for current setting
double estimateEmb (double * p, ui degree, ui root_index,double score){
	double sum_p = 0;
	...
	double sample_inverse_possiblity = sum_p / p[root_index];
//	std::cout <<"sum_p: "<<  sum_p <<" p[root_index] " <<p[root_index] <<std::endl;
//	std::cout <<"test val: "<<   score*sample_inverse_possiblity <<std::endl;
	return score*sample_inverse_possiblity;
}
//CPU memory
/*allocate candidate*/
...
//wandor join
ui WJ(Edges ***edge_matrix, ui **valid_candidate_idx, ui **candidates, ui *candidates_count, ui* idx_count, ui *order, ui **bn, ui *bn_count, bool* &sampling_visited_vertices, ui * embedding,ui *idx_embedding, ui *temp_buffer,ui root_idx,ui cur_depth ,ui max_depth,ui step, double &workloads)
{
	ui sample_depth = cur_depth;
	/* sampling one path rooted at candidate index "root_idx" then compute a score */
	ui score = 1;
	...
	while (sample_depth < cur_depth+ step  && sample_depth < max_depth){
		...
		if (sample_depth ==  cur_depth+ step -1 || sample_depth == max_depth-1) {
			...
			generateValidCandidateWJ(sample_depth, idx_embedding, idx_count, valid_candidate_idx, edge_matrix, bn,
													bn_count, order, temp_buffer);

		}

	}
	// check whether this path is correct
	// query graph
	for (int j = 1;j < max_depth; ++j ){
		u = order[j];
		ui v_idx = idx_embedding[u];
		// check all non-tree edge links to u
		for (ui i = 1; i < bn_count[j]; ++i) {
			VertexID current_bn = bn[j][i];
			Edges& current_edge = *edge_matrix[current_bn][u];
			ui current_index_id = idx_embedding[current_bn];
			...
			ui* current_candidates = current_edge.edge_ + current_edge.offset_[current_index_id];
			//check if v is valid
			if(SearchCheck(current_candidates,current_candidates_count,v_idx)){
...
template <const ui blocksize>
ui helpWJ (const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix, ui **candidates, ui *candidates_count,
        ui *order, size_t output_limit_num, size_t &call_count, ui step, timer &record ){
	//
	record. sampling_time = 0;
	record. enumerating_time = 0;
...
template <const ui blocksize>
ui partialIntersection (const Graph *data_graph, const Graph *query_graph, Edges ***edge_matrix, ui **candidates, ui *candidates_count,
        ui *order, size_t output_limit_num, size_t &call_count, ui step, timer &record ){
	//
	record. sampling_time = 0;
	record. enumerating_time = 0;
